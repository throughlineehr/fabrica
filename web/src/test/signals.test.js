...
describe('library metadata', () => {
  it('exports the expected library', () => {
    expect(PROCESSOR_LIBRARY.map(p => p.id).sort()).toEqual([
      ...
      'period-detector', 'pulse-batcher', 'sentiment', 'sentiment-tracker',
      'splitter', 'step-detector', 'test-explainer', 'test-generator',
      'top-k-tracker', 'tracer', 'trend-detector', 'websocket-transducer',
    ])
  })
